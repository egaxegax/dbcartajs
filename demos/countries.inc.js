  ...
  for (var i=0; i<cntrylist.options.length; i++) {
    ...
    if (opt.selected) {
      var mpart = opt.value,
          ...
          abbr = window.COUNTRIES[cntryname][mpart][0],
...
function init() {
  ...
  dw.draw();
  dw2 = new dbCarta("canvasmap2");
  dw2.changeProject(203);
  dw2.scaleCarta(3);
  dw2.loadCarta(window.CONTINENTS);
  dw2.loadCarta(meridians);
  dw2.draw();
  dw3 = new dbCarta("canvasmap3");
  dw3.changeProject(201);
  dw3.scaleCarta(2);
  dw3.loadCarta(window.CONTINENTS);
  dw3.loadCarta(meridians);
  dw3.draw();
  dw4 = new dbCarta("canvasmap4");
  dw4.changeProject(101);
  dw4.scaleCarta(3);
  dw4.loadCarta(window.CONTINENTS);
  dw4.loadCarta(meridians);
  dw4.draw();
  window.CONTINENTS = undefined;

  for (var cntryname in window.COUNTRIES) {
    el = document.createElement("optgroup");
    el.label = cntryname;
    for (var mpart in window.COUNTRIES[cntryname]) {
      el2 = document.createElement("option");
      el2.value = mpart;
      el2.appendChild(document.createTextNode(window.COUNTRIES[cntryname][mpart][0]));
      el.appendChild(el2);
    }
